            ...
            <tbody>
                <template v-if="!pending && rows">
                    <tr v-for="row in rows.data" :key="row.id" class="text-sm">
                        ...
                        <td>
                            <FormSwitch :id="'row-active-' + row.id" v-model="row.active" :disabled="serverParams.deleted" @change="useToggleSwitch(row.id, 'active', 'event-help-center')" />
                        </td>
                        ...
                        <div class="box p-5 !shadow-none mt-5">
                            <div class="space-y-4">
                                <div v-for="(item, index) in item.list" :key="index" class="flex items-start gap-5">
                                    <FormInputField v-model="item.name" class="grow" type="text" :name="'item-name-' + index" placeholder="Content" />
                                    <FormSwitch
                                        v-model="item.active"
