...
const rules = ref({
    ...
    publicShow: {},
    delegatesCount: { numeric, required },
    type: { required },
    active: {},
...
const fetchItem = async (id) => {
    const { data, error } = await useApiFetch(`/api/room/${id}`, {
        lazy: true,
    });
    if (data.value) {
...
async function updateItem() {
    const { data, error } = await useApiFetch(`/api/room/${item.value?.id}`, {
        method: 'PATCH',
        body: item,
        lazy: true,
    });
...
async function addItem() {
    const { data, error } = await useApiFetch(`/api/room`, {
        method: 'POST',
        body: item,
        lazy: true,
    });
...
async function deleteItems() {
    ...
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/room/delete`, {
...
async function forceDeleteItems() {
    const confirmed = confirm('Are you sure you want to delete this item?');
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/room/force-delete`, {
            body: { items: selectedRows.value },
            method: 'DELETE',
            lazy: true,
        });
...
async function restoreItems() {
    const confirmed = confirm('Are you sure you want to delete this item?');
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/room/restore`, {
            body: { items: selectedRows.value },
            method: 'POST',
            lazy: true,
        });
...
const types = ref([
    { name: 'Single Room', id: 'single' },
    { name: 'Double Room', id: 'double' },
    { name: 'Other', id: 'other' },
]);

const addRoomTypeRow = () => {
    item.value.publicTypes.push({
        type: null,
        price: null,
    });
};
const removeRoomTypeRow = (index) => {
    item.value.publicTypes.splice(index, 1);
};
        ...
        <table class="table table-report font-light">
            <thead>
                <tr class="uppercase text-sm">
                    ...
                    <th class="text-center whitespace-nowrap">Public Show</th>
                    ...
                    <tr v-for="row in rows.data" :key="row.id" class="text-sm">
                        ...
                        <td class="font-normal">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="font-light flex items-center gap-3 mt-0.5">
                                        <div class="whitespace-nowrap">{{ types.find((_) => _.id === row.type).name }}</div>
                                        <div class="text-xs px-1.5 py-0.5 rounded-full font-medium border bg-slate-100">{{ row.delegatesCount }}</div>
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td>
                            <div>
                                ...
                                <span class="font-light ml-2">USD</span>
                            </div>
                        </td>
                        <td>
                ...
                <div class="grid lg:grid-cols-12 gap-5 items-start">
                    ...
                    <div class="lg:col-span-8 grid grid-cols-12 gap-5">
                        ...
                        <div class="lg:col-span-12">
                            <div class="form-label opacity-75">Room Public Types</div>
                            <p class="text-xs font-light">This will be Shown in Hotel Page for Public View</p>
                            <div class="bg-white rounded-xl border p-5 mt-2">
                                <div class="space-y-4">
