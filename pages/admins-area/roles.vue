...
const fetchItem = async (id) => {
    const { data, error } = await useApiFetch(`/api/role/${id}`, {
        lazy: true,
    });
    if (data.value) {
...
async function updateItem() {
    const { data, error } = await useApiFetch(`/api/role/${item.value?.id}`, {
        method: 'PATCH',
        body: item,
        lazy: true,
    });
...
async function deleteItems() {
    ...
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/role/delete`, {
...
async function forceDeleteItems() {
    const confirmed = confirm('Are you sure you want to delete this item?');
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/role/force-delete`, {
            body: { items: selectedRows.value },
            method: 'DELETE',
            lazy: true,
        });
...
async function restoreItems() {
    const confirmed = confirm('Are you sure you want to delete this item?');
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/role/restore`, {
            body: { items: selectedRows.value },
            method: 'POST',
            lazy: true,
        });
...
const { data: permissions, refresh: refreshPermissions } = await useApiFetch('/api/permission/index', {
    method: 'POST',
    body: {
        filters: {},
        orderBy: 'name',
