            ...
            <tbody>
                <template v-if="!pending && rows">
                    <tr v-for="row in rows.data" :key="row.id" class="text-sm">
                        ...
                        <td>
                            <FormSwitch :id="'row-active-' + row.id" v-model="row.active" :disabled="serverParams.deleted" @change="useToggleSwitch(row.id, 'active', 'network')" />
                        </td>
                        <td>
                            <FormSwitch :id="'row-collection-' + row.id" v-model="row.collection" :disabled="serverParams.deleted" @change="useToggleSwitch(row.id, 'collection', 'network')" />
                        </td>
            ...
            <template #content>
                <div class="grid lg:grid-cols-12 gap-5 items-start">
                    <div class="lg:col-span-4">
                        <FormUploader v-model="item.image" :errors="v$.image.$errors" :allowed-types="['image', 'svg']" label="Icon" name="image" />
                    </div>
