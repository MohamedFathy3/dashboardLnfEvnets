...
const fetchItem = async (id) => {
    const { data, error } = await useApiFetch(`/api/event-day/${id}`, {
        lazy: true,
    });
    if (data.value) {
...
async function updateItem() {
    const { data, error } = await useApiFetch(`/api/event-day/${item.value?.id}`, {
        method: 'PATCH',
        body: item,
        lazy: true,
    });
        ...
        <TheModal :open-modal="isOpen" size="5xl" @close-modal="closeModal()">
            ...
            <template #content>
                <div class="grid lg:grid-cols-12 gap-5 items-start">
                    ...
                    <FormInputField v-model="item.date" readonly disabled :errors="v$.date.$errors" class="lg:col-span-6" label="Order" name="order-id" placeholder="Order Number" />
                </div>
            </template>
            <template #footer>
