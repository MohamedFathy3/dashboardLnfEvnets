...
const fetchItem = async (id) => {
    const { data, error } = await useApiFetch(`/api/program/${id}`, {
        lazy: true,
    });
    if (data.value) {
...
async function updateItem() {
    const { data, error } = await useApiFetch(`/api/program/${item.value?.id}`, {
        method: 'PATCH',
        body: item,
        lazy: true,
    });
...
async function addItem() {
    const { data, error } = await useApiFetch(`/api/program`, {
        method: 'POST',
        body: item,
        lazy: true,
    });
...
async function deleteItems() {
    ...
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/program/delete`, {
...
<template>
    <div class="flex flex-col gap-8">
        ...
        <div class="grid lg:grid-cols-12 gap-5 items-center p-5 bg-white border rounded-2xl">
            ...
            <FormSelectField v-model="filter.dayId" class="xl:col-span-6 lg:col-span-6" labelvalue="name" :clearable="false" keyvalue="id" :select-data="days?.data" name="day-id" placeholder="Day" />
                ...
                <template v-if="!pending && rows">
                    <tr v-for="row in rows.data" :key="row.id" class="text-sm">
                        ...
                        <td class="font-normal">
                            <div>{{ convertTo12HourFormat(row.from) }}</div>
                            <div class="font-light opacity-75">{{ convertTo12HourFormat(row.to) }}</div>
                        </td>
                        <td>
                        ...
                        <td class="font-medium">
                            <div class="opacity-75">{{ row.name }}</div>
                            <div class="opacity-75 font-light">{{ row.dressCode }}</div>
                        </td>
                        <td>
        ...
        <TheModal :open-modal="isOpen" size="5xl" @close-modal="closeModal()">
            ...
            <template #content>
                <div class="grid lg:grid-cols-12 gap-5 items-start">
                    <FormSelectField v-model="item.from" :errors="v$.from.$errors" labelvalue="name" keyvalue="value" :select-data="generateTimeOptions" class="lg:col-span-6" label="From" name="program-from" placeholder="From" />
                    <FormSelectField v-model="item.to" :errors="v$.to.$errors" labelvalue="name" keyvalue="value" :select-data="generateTimeOptions" class="lg:col-span-6" label="To" name="program-to" placeholder="To" />
                    <FormInputField v-model="item.name" :errors="v$.name.$errors" class="col-span-12" label="Name" name="name" placeholder="Name" />
                    <FormInputField v-model="item.location" :errors="v$.location.$errors" class="col-span-12" label="Location" name="location" placeholder="Location" />
