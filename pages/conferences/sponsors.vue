...
const fetchItem = async (id) => {
    const { data, error } = await useApiFetch(`/api/sponsor/${id}`, {
        lazy: true,
    });
    if (data.value) {
...
async function updateItem() {
    const { data, error } = await useApiFetch(`/api/sponsor/${item.value?.id}`, {
        method: 'PATCH',
        body: item,
        lazy: true,
    });
...
async function addItem() {
    const { data, error } = await useApiFetch(`/api/sponsor`, {
        method: 'POST',
        body: item,
        lazy: true,
    });
...
async function deleteItems() {
    ...
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/sponsor/delete`, {
                    ...
                    <tr v-for="row in rows.data" :key="row.id" class="text-sm">
                        ...
                        <td class="font-normal">
                            <div class="flex items-center gap-3">
                                <NuxtImg :src="row.imageUrl" :alt="row.name" :title="row.name" class="w-20 h-10 bg-white object-contain p-1 !rounded-md" />
                                <div>
                                    <div class="font-normal">{{ row.name }}</div>
                                    <div class="font-light text-sm opacity-75">{{ row.sponsorshipName }}</div>
                                </div>
                            </div>
                        </td>
                        <td>
            ...
            <template #content>
                <div class="grid lg:grid-cols-12 gap-5 items-start">
                    <div class="lg:col-span-4">
                        <FormUploader v-model="item.image" :errors="v$.image.$errors" :allowed-types="['image', 'svg']" label="Logo" name="image" />
                    </div>
                    <div class="lg:col-span-8 grid lg:grid-cols-12 gap-5 items-center">
                        ...
                        <FormInputField v-model="item.sponsorshipName" :errors="v$.sponsorshipName.$errors" class="lg:col-span-12" label="Sponsorship Name" name="sponsorship-name" placeholder="Sponsorship Name" />
                        ...
                        <FormInputField v-model="item.url" :errors="v$.url.$errors" class="lg:col-span-12" label="Url" name="url" placeholder="Url" />
                        <FormInputField v-model="item.orderId" :errors="v$.orderId.$errors" class="lg:col-span-12" label="Position" name="order-id" placeholder="Position" />
                        <FormSwitch id="active" v-model="item.active" name="active" :errors="v$.active.$errors" class="lg:col-span-6" label="Active" />
                        <FormSwitch id="active" v-model="item.eventSponsor" name="event-sponsor" :errors="v$.eventSponsor.$errors" class="lg:col-span-6" label="Event Sponsor" />
                    </div>
                </div>
            </template>
            <template #footer>
