...
async function deleteItems() {
    ...
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/trade-reference/delete`, {
...
async function forceDeleteItems() {
    const confirmed = confirm('Are you sure you want to delete this item?');
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/trade-reference/force-delete`, {
            body: { items: selectedRows.value },
            method: 'DELETE',
            lazy: true,
        });
...
async function restoreItems() {
    const confirmed = confirm('Are you sure you want to delete this item?');
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/trade-reference/restore`, {
            body: { items: selectedRows.value },
            method: 'POST',
            lazy: true,
        });
...
<template>
    <div v-if="usePermissionCheck(['network_trade_reference_list'])" class="flex flex-col gap-8">
        <!-- Page Title & Action Buttons -->
        <div class="md:flex md:items-center md:justify-between md:gap-5">
            <div class="flex items-center gap-2">
                <Icon name="solar:bill-list-outline" class="size-5 opacity-75" />
                <div>{{ serverParams.deleted ? 'Deleted Trade References' : 'Trade References' }}</div>
            </div>
            <div class="md:flex md:items-center md:gap-5 md:space-y-0 space-y-5">
                <template v-if="selectedRows.length > 0">
                    <template v-if="serverParams.deleted">
                        <button v-if="usePermissionCheck(['network_trade_reference_force_delete'])" class="btn btn-danger btn-rounded px-6 btn-sm gap-3 md:w-fit w-full md:mt-0 mt-5" @click="forceDeleteItems">
                            <Icon name="solar:trash-bin-minimalistic-line-duotone" class="size-5 opacity-75" />
                            Delete Permanently
                        </button>
                    </template>
                    <template v-else>
                        <button v-if="usePermissionCheck(['network_trade_reference_delete'])" class="btn btn-danger btn-rounded px-6 btn-sm gap-3 md:w-fit w-full md:mt-0 mt-5" @click="deleteItems">
                            <Icon name="solar:trash-bin-minimalistic-line-duotone" class="size-5 opacity-75" />
                            Delete Items
                        </button>
                    </template>
                    <template v-if="serverParams.deleted">
                        <button v-if="usePermissionCheck(['network_trade_reference_restore'])" class="btn btn-success btn-rounded px-6 btn-sm gap-3 md:w-fit w-full md:mt-0 mt-5" @click="restoreItems">
                ...
                <!--                </button>-->
                <button
                    v-if="usePermissionCheck(['network_trade_reference_delete', 'network_trade_reference_force_delete', 'network_trade_reference_restore'])"
                    ...
                    <tr v-for="row in rows.data" :key="row.id" class="text-sm">
                        ...
                        <td class="text-sm font-normal whitespace-nowrap">
                            <div v-if="row.company" class="flex items-center gap-3">
                                <NuxtImg v-if="row.company.imageUrl" :src="row.company.imageUrl" class="h-10 !rounded-md w-20 object-contain p-1 shrink-0" />
                            ...
                            <div v-else class="font-light">---</div>
                        </td>
